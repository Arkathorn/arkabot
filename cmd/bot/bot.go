...
// Play represents an individual use of the !arkabot command
type Play struct {
	GuildID   string
	ChannelID string
	UserID    string
	...
	// If true, this was a forced play using a specific arkabot sound name
	Forced bool
}
...
func trackSoundStats(play *Play) {
	...
	_, err := rcli.Pipelined(func(pipe *redis.Pipeline) error {
		...
		base := fmt.Sprintf("arkabot:%s", baseChar)
		pipe.Incr("arkabot:total")
...
func onReady(s *discordgo.Session, event *discordgo.Ready) {
	...
	s.UpdateStatus(0, "arkabotbot.com")
}
...
func onGuildCreate(s *discordgo.Session, event *discordgo.GuildCreate) {
	...
	for _, channel := range event.Guild.Channels {
		if channel.ID == event.Guild.ID {
			s.ChannelMessageSend(channel.ID, "**ARKABOT BOT READY FOR HORNING. TYPE `!AIRHORN` WHILE IN A VOICE CHANNEL TO ACTIVATE**")
			return
		}
	}
}
...
func calculateAirhornsPerSecond(cid string) {
	current, _ := strconv.Atoi(rcli.Get("arkabot:a:total").Val())
	time.Sleep(time.Second * 10)
	latest, _ := strconv.Atoi(rcli.Get("arkabot:a:total").Val())
...
func displayUserStats(cid, uid string) {
	keys, err := rcli.Keys(fmt.Sprintf("arkabot:*:user:%s:sound:*", uid)).Result()
	if err != nil {
		return
	}
...
func displayServerStats(cid, sid string) {
	keys, err := rcli.Keys(fmt.Sprintf("arkabot:*:guild:%s:sound:*", sid)).Result()
	if err != nil {
		return
	}
...
func arkabotBomb(cid string, guild *discordgo.Guild, user *discordgo.User, cs string) {
...
func handleBotControlMessages(s *discordgo.Session, m *discordgo.MessageCreate, parts []string, g *discordgo.Guild) {
	if scontains(parts[1], "status") {
		...
		arkabotBomb(m.ChannelID, g, utilGetMentioned(s, m), parts[3])
...
func onMessageCreate(s *discordgo.Session, m *discordgo.MessageCreate) {
	...
	if parts[0] == "!t" && parts[1] == "list" {
		s.ChannelMessageSend(channel.ID, "**FEATURE NOT YET IMPLEMENTED**")
	}
...
func main() {
	...
	// We're running!
	log.Info("ARKABOTBOT is ready to horn it up.")
