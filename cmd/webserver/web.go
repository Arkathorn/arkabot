...
func NewCountUpdate() *CountUpdate {
	...
	errors, err := rcli.Pipelined(func(pipe *redis.Pipeline) error {
		totalCmd = pipe.Get("arkabot:a:total")
		usersCmd = pipe.SCard("arkabot:a:users")
		guildsCmd = pipe.SCard("arkabot:a:guilds")
		chansCmd = pipe.SCard("arkabot:a:channels")
		secretCmd = pipe.Get("arkabot:a:sound:truck")
		return nil
	})
...
func main() {
	...
	oauthConf = &oauth2.Config{
		...
		Endpoint:     endpoint,
		RedirectURL:  "https://arkabotbot.com/callback",
	}
