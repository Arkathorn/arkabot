...
const Content = React.createClass({
  ...
  getCenter(): React.Element {
    if (this.state.isMobile) {
      return <img className="video-arkabot" src={Constants.Image.ISLAND_ARKABOT_MOBILE} />;
    }
    else {
      return (
        <video
          preload
          className="video-arkabot"
          ref="video"
          onClick={this.playVideo}>
          <source src={Constants.Video.ARKABOT} type="video/mp4" />
          <source src={Constants.Video.ARKABOT_OGV} type="video/ogg; codecs=theora, vorbis" />
          <source src={Constants.Video.ARKABOT_WEBM} type="video/webm; codecs=vp8, vorbis" />
          <audio preload src={Constants.Audio.ARKABOT} type="audio/wav" ref="audio" />
        </video>
      );
    }
  },
